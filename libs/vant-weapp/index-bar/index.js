...
VantComponent({
  ...
  created() {
    ...
    wx.getSystemInfo({
      success: function (res) {
        ...
        that.setData({
          ios: res.platform === 'ios'
        });
        if (res.model.includes('iPhone 12 Pro Max')) {
          that.setData({
            iphone12: true
          });
        }
  ...
  methods: {
    ...
    onTouchMove(event) {
      ...
      //   }
      //   if (index >= 0 || index <= sidebarLength - 1) {
      //     index = sidebarLength - 1
      //   }
      if (index < 0 || index >= sidebarLength) return;
